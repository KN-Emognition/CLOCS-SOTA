...
from prepare_miscellaneous import (
    obtain_criterion,
    print_metrics,
    track_metrics,
    save_metrics,
...
def train_model(
    ...
    """ End """
    model, optimizer, device = load_initial_model_contrastive(
        cnn_network_contrastive,
        phases,
        model_path_dir,
        saved_weights,
    ...
    dataloader, operations = load_initial_data_contrastive(
        basepath_to_data,
        phases,
        fraction,
        inferences,
    ...
    # total_labelled_samples = len(dataloaders_list['train'].batch_sampler.sampler.data_source.label_array)

    while stop_counter <= patience and epoch_count < num_epochs:
        if "train" in phases or "valid" in phases:
            print("-" * 10)
            ...
            if inference == False:
                ...
                if (
                    ...
                    if "train" in phases:
                        ...
                        save_patient_representation(
                            save_path_dir, patient_rep_dict, trial_to_run
                        )
                    stop_counter = 0
                elif phase == "valid" and epoch_loss >= best_loss:
                    stop_counter += 1

                metrics_dict = track_metrics(
                    metrics_dict, results_dictionary, phase, epoch_count
                )

        epoch_count += 1
        if "train" not in phases:
            break  # from while loop
        elif "train" in phases and "obtain_representation" in downstream_task:
            break
